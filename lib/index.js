...
function apply(ctx, config) {
	...
	ctx.inject(["settings"], (settingsCtx) => {
		...
		settingsCtx.settings.installSection(ctx, SETTINGS_NAMESPACE, Config, resolved, {
			...
			onChange: () => {
				const next = source();
				settingsCtx.logger.debug("dsh-node-accent: settings updated, paint icon=%s title=%s tools=%d", String(next.paintIcon), String(next.paintTitle), Object.keys(next.toolColors).length);
			}
		});
	});
}
